...
class PagingViewModel(app: Application) : AndroidViewModel(app) {
    val dao = PagingDb.get(app).pagingDao()

    /**
     * Google use -ktx Kotlin extension functions here,
     * otherwise you would use LivePagedListBuilder(),
     * and PagedList.Config.Builder()
     */
    val allPaging = dao.allPagingByName().toLiveData(Config(
            /**
    ...
    fun insert(text: CharSequence) = ioThread {
        dao.insert(Paging(id = 0, name = text.toString()))
    }

    fun remove(paging: Paging) = ioThread {
        dao.delete(paging)
    }
}